...
void HariMain(void)
{
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
		...
		putfonts8_asc(binfo->vram, binfo->scrnx, binfo->scrnx-185, binfo->scrny-77, 0, "WBoxOS Operating System");
		//putfonts8_asc(binfo->vram, binfo->scrnx, 655, 539, COL8_000000, "Version 0.1 [Beta]");
		putfonts8_asc(binfo->vram, binfo->scrnx, binfo->scrnx-145, binfo->scrny-61, 0, "Version 0.1 [Beta]");

		//putfonts8_asc(binfo->vram, binfo->scrnx, 385, 555, COL8_000000, "WARNING:THIS IS A BETA VERSION THAT MAY BE UNSTABLE.");
		putfonts8_asc(binfo->vram, binfo->scrnx, binfo->scrnx-415, binfo->scrny-45, 0, "WARNING:THIS IS A BETA VERSION THAT MAY BE UNSTABLE.");
		//?示日期
		sprintf(s, "%02d/%02d/%02d %s", get_year()%100, get_mon_hex(), get_day_of_month(),result);
		putfonts8_asc_sht(sht_back, binfo->scrnx - 150, binfo->scrny -20, 0, COL8_FFFFFF, s, 15);
		//?示??
		sprintf(s, "%02d:%02d", get_hour_hex(), get_min_hex());
		putfonts8_asc_sht(sht_back, binfo->scrnx - 45, binfo->scrny -20, 0, COL8_FFFFFF, s, 5);
